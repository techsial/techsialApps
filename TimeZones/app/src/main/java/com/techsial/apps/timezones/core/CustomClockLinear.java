...
public class CustomClockLinear extends LinearLayout {
    ...
    public void setTimeZoneModel(TimeZoneModel timeZoneModel) {
        ...
        long gmtVal = TimeUnit.HOURS.convert(mGMTOffset, TimeUnit.MILLISECONDS);
        String strGMTVal;
        if (gmtVal >= 0) {
            strGMTVal = " + " + gmtVal;
        } else {
            strGMTVal = " - " + (gmtVal + "").substring(1);
        }
    ...
    private void updateTime() {
        try {
            ...
            if (timeZoneModel.isMorning()) {
                ...
                this.setBackgroundResource(R.drawable.day_bg);
            } else {
                customHolder.timeImage.setImageDrawable(mContext.getResources().getDrawable(R.drawable.moon));
                this.setBackgroundResource(R.drawable.night_bg);
            }
...
//        customHolder.date.setText(dF.format(mCalendar.getTime()));
            ...
            customHolder.date.setText(dF.format(mCalendar.getTime()) + "\n" + daysArray[mCalendar.get(Calendar.DAY_OF_WEEK) % 7]);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
